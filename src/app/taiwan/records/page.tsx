...
export default function TaiwanRecordsPage() {
  ...
  const fetchRecords = async () => {
    setLoading(true);
    // 台湾仓调用记录：筛选 sku 包含台湾仓前缀的记录
    let query = supabase.from('inventory_records').select('*').ilike('sku', 'TW-%');
          ...
          <table className="w-full text-sm text-center">
            <thead className="bg-white border-y border-gray-200 text-gray-700 font-bold">
              <tr>
                ...
                <th className="px-4 py-3 whitespace-nowrap">訂單編號</th>
                <th className="px-4 py-3 whitespace-nowrap">SKU/商品名</th>
                <th className="px-4 py-3 whitespace-nowrap">調用數量</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">暂无台湾仓调用记录</td></tr>
              ) : records.map(r => (
