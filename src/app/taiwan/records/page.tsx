...
export default function TaiwanRecordsPage() {
  ...
  const columns: Column<InventoryRecord>[] = [
    ...
    { key: 'order_number', title: '訂單編號', className: 'text-center', render: r => <span className="text-blue-600 text-xs">{r.order_number || '-'}</span> },
    { key: 'sku', title: 'SKU/商品名', className: 'text-center', render: r => <span className="text-gray-700">{r.sku || '-'}</span> },
    { key: 'quantity', title: '調用數量', className: 'text-center', render: r => <span className="font-bold text-gray-800">{r.quantity}</span> },
    ...
    <div className="flex flex-col gap-4">
      <div className="card">
        <div className="p-4 border-b border-gray-100 flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold text-gray-800">臺灣倉庫調用記錄</h1>
        </div>
        <div className="p-4">
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <input type="text" placeholder="訂單編號" value={orderFilter} onChange={e => setOrderFilter(e.target.value)}
              className="w-full sm:w-64 h-9 px-3 text-sm border border-gray-300 rounded focus:border-[#3c8dbc] focus:outline-none" />
      ...
      <div className="card flex flex-col">
        <div className="p-4 border-b border-gray-100 flex items-center gap-2">
          <h2 className="text-lg font-bold text-gray-800">調用記錄</h2>
          <RefreshCw
            className={`w-4 h-4 text-[#3c8dbc] cursor-pointer hover:rotate-180 transition-transform duration-500 ${loading ? 'animate-spin' : ''}`}
            onClick={refresh}
          />
        </div>
        <DataTable columns={columns} data={records} loading={loading} error={error} emptyText="暂无台湾仓调用记录" checkbox={false}
