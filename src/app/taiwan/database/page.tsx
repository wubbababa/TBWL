...
export default function TaiwanDatabasePage() {
  ...
  const filterFn = useCallback((query: Parameters<typeof Array.isArray>[0]) => {
    let q = query;
    if (searchText) q = q.or(`name.ilike.%${searchText}%,sku.ilike.%${searchText}%`);
    // Only show Taiwan warehouse products
    q = q.in('store_name', ['台北仓', '台中仓', '高雄仓']);
    return q;
  }, [searchText]);
  ...
  const columns: Column<Product>[] = [
    { key: 'store_name', title: '仓点', className: 'text-center' },
    ...
    <div className="flex flex-col gap-4">
      <div className="card">
        <div className="p-4 flex flex-wrap gap-3 items-center">
          ...
          <button onClick={handleCopy} disabled={selectedIds.length === 0 || copying}
            className="flex items-center gap-1.5 h-9 px-4 bg-[#3c8dbc] text-white text-sm rounded hover:bg-[#367fa9] disabled:opacity-50 disabled:cursor-not-allowed">
            <Copy className="w-4 h-4" />
            <span>{copying ? '拷贝中...' : selectedIds.length > 0 ? `拷贝至臺灣庫存商品(${selectedIds.length})` : '拷贝至臺灣庫存商品'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="card flex flex-col">
        <div className="p-4 border-b border-gray-100 flex items-center gap-2">
          <h2 className="text-lg font-bold text-gray-800">臺灣商品庫列表</h2>
