...
export function CreateInventoryProductModal({ onClose, onCreated }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setToast({ type: 'success', message: '商品创建成功' });
      setTimeout(() => {
        onCreated();
        onClose();
      }, 800);
    ...
    >
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 sticky top-0 bg-white z-10">
          <h2 className="text-base font-bold text-gray-800">创建库存商品</h2>
