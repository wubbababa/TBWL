...
export const OrderTable = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      console.error('Supabase fetch error full object:', err);
      ...
      const errorMessage = err.message || (typeof err === 'string' ? err : '获取订单数据失败');
      setError(errorMessage);
      
      if (errorMessage === 'Failed to fetch') {
        setError('网络请求失败。请检查：1. Supabase URL 是否正确；2. 环境变量是否已设置；3. 网络连接或是否被插件拦截。');
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="bg-white border border-gray-200 rounded shadow-sm overflow-hidden min-h-[400px]">
      ...
      <div className="flex justify-between items-center p-2 bg-gray-50 border-b border-gray-200 px-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="p-1.5 hover:bg-gray-200 rounded transition-colors"><Maximize2 className="w-4 h-4 text-gray-600" /></button>
          <button className="p-1.5 hover:bg-gray-200 rounded transition-colors"><List className="w-4 h-4 text-gray-600" /></button>
        </div>
      </div>
        ...
        <table className="w-full text-sm text-left border-collapse">
          <thead className="bg-[#f9fafb] text-gray-700 font-bold border-b border-gray-200">
            <tr>
              ...
              {columns.map((col, idx) => (
                <th key={col} className={`px-4 py-3 border-r border-gray-200 last:border-r-0 ${idx === 2 ? 'w-1/4' : ''}`}>
                  {col}
                </th>
              ))}
            </tr>
            ...
            {loading ? (
              ...
              orders.map((order) => (
                <tr key={order.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 border-r border-gray-200">{order.shipping_method}</td>
                  <td className="px-4 py-3 border-r border-gray-200 truncate max-w-[200px]">{order.product_list}</td>
                  <td className="px-4 py-3 border-r border-gray-200 italic text-gray-400">{order.remarks || '-'}</td>
                  <td className="px-4 py-3 border-r border-gray-200">
                    <button className="text-blue-500 hover:text-blue-700 font-bold">详情</button>
                  </td>
                  <td className="px-4 py-3 border-r border-gray-200">
                ...
                <td colSpan={8} className="px-4 py-20 text-center text-gray-400">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="text-gray-500 font-medium">没有找到匹配的记录</span>
                      <span className="text-xs">请确保 Supabase 中 `orders` 表有数据且已开启 RLS 权限。</span>
                    </div>
                  </div>
                </td>
              </tr>
